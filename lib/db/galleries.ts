...
export async function getGalleriesList(workspaceId: number = 1): Promise<any[]> {
  const galleries = await query<any>(
    `SELECT g.*, 
            ...
            COUNT(DISTINCT gp.photo_id) as photos_count,
            COUNT(DISTINCT gv.id) as views_count
     FROM galleries_gallery g
     INNER JOIN events_event e ON e.id = g.event_id AND e.workspace_id = $1
  ...
  const enriched = galleries.map((g) => {
    ...
    return {
      ...g,
      photos_count: Number(g.photos_count) || 0,
      views_count: Number(g.views_count) || 0,
      thumbnailUrl,
    };
  });
...
export async function recordGalleryView(galleryId: number, ipAddress: string, userAgent: string, deviceType: string, workspaceId: number = 1): Promise<void> {
  const now = new Date().toISOString();
  await query(
    `INSERT INTO galleries_galleryview (workspace_id, gallery_id, viewed_at, ip_address, user_agent, device_type)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [workspaceId, galleryId, now, ipAddress || "127.0.0.1", userAgent || "", deviceType || "Web Client"]
  );
}
...
export async function createOrUpdateGallery(data: {
  ...
  // Verify event exists and belongs to the requested workspace
  const event = await queryOne<{ id: number; workspace_id: number }>(
    `SELECT id, workspace_id FROM events_event WHERE id = $1`,
    [data.eventId]
  );
  if (!event || (data.workspaceId && event.workspace_id !== data.workspaceId)) {
  ...
  let gallery = await queryOne<Gallery>(
    `SELECT * FROM galleries_gallery WHERE event_id = $1 AND workspace_id = $2`,
    [data.eventId, workspaceId]
  );

  if (!gallery) {
    gallery = await queryOne<Gallery>(
      `INSERT INTO galleries_gallery (workspace_id, event_id, title, slug, pin_hash, pin_code, is_published, published_at, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, true, $7, $8, $9)
       RETURNING *`,
      [workspaceId, data.eventId, data.title, data.slug, pinHash, pinCode, now, now, now]
    );
  } else {
    gallery = await queryOne<Gallery>(
      `UPDATE galleries_gallery 
       ...
       RETURNING *`,
      [data.title, data.slug, pinHash, pinCode, now, now, gallery.id, workspaceId]
    );
  }
...
export async function getGalleryPhotoIds(eventId: number, workspaceId?: number): Promise<number[]> {
  const sql = workspaceId
    ? `SELECT gp.photo_id
       FROM galleries_galleryphoto gp
       INNER JOIN galleries_gallery g ON g.id = gp.gallery_id
       WHERE g.event_id = $1 AND g.workspace_id = $2`
    : `SELECT gp.photo_id
       FROM galleries_galleryphoto gp
       INNER JOIN galleries_gallery g ON g.id = gp.gallery_id
       WHERE g.event_id = $1`;
  const params = workspaceId ? [eventId, workspaceId] : [eventId];
  const rows = await query<{ photo_id: number }>(sql, params);
...
export async function updateGalleryPhotoSelection(
  ...
  const sql = workspaceId
    ? `SELECT id FROM galleries_gallery WHERE event_id = $1 AND workspace_id = $2 LIMIT 1`
    : `SELECT id FROM galleries_gallery WHERE event_id = $1 LIMIT 1`;
  const params = workspaceId ? [eventId, workspaceId] : [eventId];
  const gallery = await queryOne<{ id: number }>(sql, params);
  ...
  await query(
    `UPDATE galleries_gallery SET updated_at = $1 WHERE id = $2`,
    [now, gallery.id]
  );
